    ...
    <div className="container">
      <nav className="navbar">
        ...
        <div className="navbar-center">
          ...
          {usuario && usuario.Rol === 'Cliente' && (
            <button className={vista === 'perfil' ? 'active' : ''} onClick={() => setVista('perfil')}>Mi Perfil y Pedidos</button>
          )}
        </div>
        <div className="navbar-right">
          <button onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} style={{ background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer' }}>
            {theme === 'light' ? '🌙' : '☀️'}
          </button>
          {usuario ? (
            <>
            ...
            <button className="btn" style={{ width: 'auto' }} onClick={() => setAuthModal(true)}>Iniciar Sesión</button>
          )}
        </div>
      </nav>
          ...
          <div className="modal-body">
            ...
            {!isRegister ? (
              <form onSubmit={handleLogin}>
                ...
                <input placeholder="Usuario" value={loginForm.Usuario} onChange={e => setLoginForm({...loginForm, Usuario: e.target.value})} style={{ marginBottom: '15px' }} required />
                <input type="password" placeholder="Contraseña" value={loginForm.Password} onChange={e => setLoginForm({...loginForm, Password: e.target.value})} style={{ marginBottom: '20px' }} required />
                <button type="submit" className="btn" style={{ marginBottom: '15px' }}>Entrar</button>
                <p style={{ textAlign: 'center', fontSize: '0.9rem' }}>¿No tienes cuenta? <span style={{ color: 'var(--primary-color)', cursor: 'pointer', fontWeight: 'bold' }} onClick={() => setIsRegister(true)}>Regístrate aquí</span></p>
              </form>
            ) : (
              <form onSubmit={handleRegister} style={{ maxHeight: '80vh', overflowY: 'auto', paddingRight: '10px' }}>
                ...
                <input placeholder="Usuario (Login)" value={regForm.Usuario} onChange={e => setRegForm({...regForm, Usuario: e.target.value})} style={{ marginBottom: '10px' }} required />
                <input type="password" placeholder="Contraseña" value={regForm.Password} onChange={e => setRegForm({...regForm, Password: e.target.value})} style={{ marginBottom: '10px' }} required />
                <input placeholder="Nombre Completo" value={regForm.NombreCompleto} onChange={e => setRegForm({...regForm, NombreCompleto: e.target.value})} style={{ marginBottom: '15px' }} required />
                ...
                <button type="button" className="btn-danger" style={{ width: '100%' }} onClick={() => setIsRegister(false)}>Volver al Login</button>
              </form>
            )}
          </div>
        </div>
