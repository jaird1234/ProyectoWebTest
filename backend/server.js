...
// ── CATEGORÍAS ────────────────────────────────────────────────
// FIX 1: GET faltaba completamente → sin esto la tienda y el admin no cargan nada
...
// FIX 2: Agregado try/catch y NVarChar(100) correcto
...
app.get('/api/articulos', async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request()
      .query('SELECT a.*, c.Nombre AS CategoriaNombre FROM Articulo a LEFT JOIN Categorias c ON a.CategoriaId = c.Id');
    res.json(result.recordset);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// FIX 3: Agregados Descuento y Destacado que el frontend enviaba pero el backend ignoraba
app.post('/api/articulos', async (req, res) => {
  const { Nombre, Precio, Stock, CategoriaId, Imagen, Descuento, Destacado } = req.body;
  try {
    const pool = await poolPromise;
    await pool.request()
      .input('Nombre',      sql.NVarChar(100),    Nombre)
      ...
      .query('INSERT INTO Articulo (Nombre,Precio,Stock,CategoriaId,Imagen,Descuento,Destacado) VALUES (@Nombre,@Precio,@Stock,@CategoriaId,@Imagen,@Descuento,@Destacado)');
    res.status(201).json({ message: 'Creado' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// FIX 4: Agregados Descuento y Destacado en el UPDATE también
app.put('/api/articulos/:id', async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      ...
      .input('Descuento',   sql.Int,              Descuento || 0)
      .input('Destacado',   sql.Bit,              Destacado ? 1 : 0)
      .query('UPDATE Articulo SET Nombre=@Nombre, Precio=@Precio, Stock=@Stock, CategoriaId=@CategoriaId, Imagen=@Imagen, Descuento=@Descuento, Destacado=@Destacado WHERE Id=@Id');
...
app.get('/api/entregas-proveedores', async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request()
      .query('SELECT e.*, p.Nombre AS ProveedorNombre FROM EntregasProveedores e JOIN Proveedores p ON e.ProveedorId = p.Id ORDER BY e.FechaProgramada ASC');
    res.json(result.recordset);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/api/entregas-proveedores/:id/recibir', async (req, res) => {
  try {
    ...
    await pool.request()
      .input('Id', sql.Int, req.params.id)
      .query("UPDATE EntregasProveedores SET Estado='Recibida' WHERE Id=@Id");
    res.sendStatus(200);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// ── EMPLEADOS ────────────────────────────────────────────────
// FIX 5: GET faltaba completamente → lista siempre vacía en el panel admin
app.get('/api/empleados', async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request()
      .query("SELECT Id, NombreCompleto, Usuario, Rol, Telefono, Correo FROM Usuarios WHERE Rol != 'Cliente'");
    res.json(result.recordset);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// FIX 6: POST ahora hashea la contraseña (antes texto plano → error 500 en login posterior)
app.post('/api/empleados', async (req, res) => {
  ...
  try {
    ...
    await pool.request()
      ...
      .query('INSERT INTO Usuarios (NombreCompleto, Usuario, Password, Rol, Telefono, Correo) VALUES (@NombreCompleto, @Usuario, @Password, @Rol, @Telefono, @Correo)');
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// FIX 7: Agregado campo Correo que el frontend envía al editar
app.put('/api/empleados/:id', async (req, res) => {
  const { NombreCompleto, Rol, Telefono, Correo } = req.body;
  try {
    const pool = await poolPromise;
    await pool.request()
      .input('Id',             sql.Int,           req.params.id)
      ...
      .input('Telefono',       sql.NVarChar(20),  Telefono || null)
      .input('Correo',         sql.NVarChar(100), Correo   || null)
      .query('UPDATE Usuarios SET NombreCompleto=@NombreCompleto, Rol=@Rol, Telefono=@Telefono, Correo=@Correo WHERE Id=@Id');
    res.sendStatus(200);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/repartidores', async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request()
      .query("SELECT Id, NombreCompleto FROM Usuarios WHERE Rol='Repartidor'");
    res.json(result.recordset);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// ── CLIENTES ──────────────────────────────────────────────────
// FIX 8: GET faltaba completamente
app.get('/api/clientes', async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request()
      .query("SELECT Id, NombreCompleto, Usuario, Calle, Numero, Colonia, Municipio, CodigoPostal, Telefono FROM Usuarios WHERE Rol='Cliente'");
    res.json(result.recordset);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/pedidos', async (req, res) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request()
      .query('SELECT p.*, u.NombreCompleto AS Cliente, r.NombreCompleto AS Repartidor FROM Pedidos p JOIN Usuarios u ON p.ClienteId = u.Id LEFT JOIN Usuarios r ON p.RepartidorId = r.Id ORDER BY p.Fecha DESC');
    res.json(result.recordset);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
  ...
  try {
    ...
    for (const item of detalle) {
      const check = await pool.request()
        ...
        .query('SELECT Stock, Nombre FROM Articulo WHERE Id=@Id');
      if (check.recordset[0].Stock < item.Cantidad)
        return res.status(400).json({ error: `Stock insuficiente para: ${check.recordset[0].Nombre}` });
    }
